...
3-1. **스키마 사본 신선도**: 문서 옆 `schemas/*.json`은 스킬 소유 자산의 **사본**이므로 원본과 **같아야 한다**. 다르면 "사본이 낡음 — 스킬 자산으로 갱신 필요"로 보고한다. 낡은 사본을 두면 **낡은 계약으로 검증**하게 되어 지금 계약 위반을 못 잡는다(조용한 통과).
3-2. **옛 계약 흔적**: 지금은 없어진 키가 블록에 남아 있으면(예: 데이터 모델의 `source`·`usedIn`) **"옛 스키마 형식 — 스키마 이행 필요"**로 보고한다. **`version` 값으로는 구분되지 않으므로**(계약이 제자리에서 다듬어진 구간이 있다) **형태로 판별**한다. 스키마 위반만 나열하면 "필드가 빠졌다"로만 보여, 형식을 옮기면 된다는 걸 알 수 없다.
