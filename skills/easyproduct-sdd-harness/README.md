...
- **조사(audit)를 먼저** 합니다: spec-kit 유무·스택·상위 문서 형식·SDD 규범 유무·기존 게이트·**이미 깔린 spec-kit 확장**·관장 사각·하네스 설치 상태 8항목.
...
- 브라운필드는 **`warn`으로 시작**해 정리 후 `block`으로 전환합니다.
...
## 버전
- 세트 버전 `0.5.0` (버전 규칙은 저장소 [VERSIONING.md](../../VERSIONING.md), 변경 내역은 [CHANGELOG.md](../../CHANGELOG.md))
